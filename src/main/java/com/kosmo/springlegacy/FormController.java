...
/*
 ...
 @Controller
 	: 해당 클래스르 컨트롤러 사용하고 싶을 때 클래스명 앞에
 	선언한다. 패키지를 스캔할 때 해당 어노테이션이 있는 클래스를 찾아서
 	요청을 전달한다.
 @RequestMapping
 	: 요청명을 매핑한다. 요청명은 서블릿과 마찬가지고 컨텍스트 루트를
 	제외한 나머지 경로명으로 이루어진다. 요청명의 매핑 정보로 메서드를
 	호출하므로 메서드명은 큰 의미가 없다. 개발자가 구분하기 좋으 정도의
 	이름으로 설정하면 된다.
...
public class FormController {
	...
	/*
	 폼값받기3] 커맨드 객체를 이용해서 폼값 한 번에 받기
	 조건1: 쿼리스트링으로 전달되는 파라미터의 개수와 폼값을 저장할
	 	객체(DTO혹은 VO)의 멤버변수의 개수가 동일한 것을 권장한다.
	 조건2: 커맨드 객체를 사용할 때는 클래스명 앞글자를 소문자로 바꾼
	 	형태의 매개변수를 사용해야 한다.
	 	Ex) MemberDTO -> memberDTO
	 조건3: 커맨드 객체를 만들 때 파라미터의 개수는 달라도 상관 없으나
	...
	/*
	 폼값받기4] @PathVariable 어노테이션 사용하기
	 	...
	 	파라미터를 마치 경로명처럼 사용하게 되고, 이 때 주의해야 할 점은
