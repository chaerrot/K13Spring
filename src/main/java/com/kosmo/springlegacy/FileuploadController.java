...
public class FileuploadController {
	...
	public void uploadPath(HttpServletRequest req,
		...
		// request 내장 객체를 통해 서버의 물리적 경로 얻어옴
		String path = req.getSession().getServletContext()
				.getRealPath("/resources/upload");
		// upload 디렉토리는 정적 파일을 저장하기 위한 resources 하위에 생성한다.
		
		// response 내장 객체를 통해 MIME 타입을 설정한다.
	...
	/*
	 파일업로드 처리
	 	: 파일업로드는 Post 방식으로 전송해야하므로 매핑 시 method, value 두 가지
	 	속성을 모두 기술해야 한다.
	 */
	...
	public String uploadAction(Model model,
		...
		try {
			...
			while(itr.hasNext()) {
				...
				// 하나의 파일 정보를 저장한 Map을 List에 저장한다.(현재 파일 2개임)
	...
	public String uploadList(HttpServletRequest req, Model model) {
		...
		// View로 전달할 파일 목록 저장을 위해 Map컬렉션 생성
		Map<String, Integer> fileMap = new HashMap<String, Integer>();
		for (File f : fileArray) {
			// key와 value로 파일명과 파일 용량을 저장한다.
