...
/*
 ...
 컨트롤러 클래스 역할을 부여하기 위한 어노테이션으로, 스프링 컨테이너가 시작될 때
 자동으로 빈이 생성된다.
 */
@Controller
public class ServiceController {
	...
	/*
	 서비스 객체를 주입 받는다. @Autowired는 멤버변수, 생성자, setter에서
	...
	/*
	 ...
	 이 때 컨트롤러가 받은 모든 요청(request객체)을 전달한다.
	 */
	...
	/*
	 사용자가 HTML페이지에서 각 링크를 클릭할 때 전달하는 userLottoNum 파라미터를
	 lottoVO 객체가 받아서 사용한다.
	 */
	@RequestMapping("/service/myLotto.do")
	public String myLotto(LottoVO lottoVO, Model model) {
		/*
		 파라미터가 전달되는 값은 커맨드객체(VO객체)를 통해 한꺼번에 받을 수
		 있으므로 getParameter()가 필요 없다.
		 */
